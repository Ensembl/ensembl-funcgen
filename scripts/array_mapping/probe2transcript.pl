...
if($reg_file || $reg_host){
  ...
  $transcript_db = new Bio::EnsEMBL::DBSQL::DBAdaptor(
  ...
  $xref_db = new Bio::EnsEMBL::Funcgen::DBSQL::DBAdaptor('-host'   => $xref_host,
														 '-port'   => $xref_port,
														 '-user'   => $xref_user,
														 '-pass'   => $xref_pass,
														 '-dbname' => $xref_dbname,
  ...
  if ($probe_host && $probe_dbname && $probe_user) {
	...
	$probe_db = new Bio::EnsEMBL::Funcgen::DBSQL::DBAdaptor('-host'   => $probe_host,
															'-port'   => $probe_port,
															'-user'   => $probe_user,
															'-pass'   => $probe_pass,
															'-dbname' => $probe_dbname,
...
if ($health_check){
  ...
  #Add report on meta_coord entries
  #Can we actually healtcheck this properly or just print out?
  #$sql = 'SELECT


  
  $Helper->log("Healthcheck passed");
  exit 0;
}
...
if($test_slice){
  ...
  $Helper->log("Running test mode with transcript:\t$transcript_sid\n".
...
sub log_orphan_probes {
  ...
  if($test_slice || $transcript_sid){
	$Helper->log("Skipping log_orphan_probes as we are running on a test slice or single transcript");
	return;
  }
...
sub usage {
  ...
  READING ARRAYS:
   ...
   --array_host          The database server to read probe features from.
		
   [--probe_port]        The port to use for reading probe features. Defaults to 3306.
		
   --array_user          Database username for reading probe featuress.
		
   --array_pass          Password for probe_user, if required.
		
   --array_dbname        Database name to read probe features from.
