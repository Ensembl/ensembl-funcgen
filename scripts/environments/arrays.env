...
DumpSeq(){
	...
	if [ $seq_type = "GENOMIC" ]; then
		export GENOMICSEQS=${dir}/${SPECIES}_toplevel_${assm}.fasta 
 	...
 	#Can't do this if we have note made a full copy of the DB!!!
		bsub_cmd="-e ${dir}/genomic_seq.err -o ${dir}/genomic_seq.out -R\"select[mem>3500] rusage[mem=3500]\" -M3500000 "
		cmd="$EFG_PERL $ANALYSIS_SCRIPTS/sequence_dump.pl -dbhost $DNADB_HOST -dbuser $DNADB_USER -dbname $DNADB_NAME -species $SPECIES -coord_system_name toplevel -mask_repeat Dust  -mask_repeat RepeatMask  -softmask $MULTI_SPECIES -filename $GENOMICSEQS"

	elif  [ $seq_type = "TRANSCRIPT" ]; then
		#CheckVariables now done in _InitEnv

		export TRANSCRIPTSEQS=${dir}/${SPECIES}_transcripts.${BUILD}.fasta
		...
		bsub_cmd="-e ${dir}/transcript_seq.err -o ${dir}/transcript_seq.out -R\"select[mem>3500] rusage[mem=3500]\" -M3500000 "
...
RunTranscriptXrefs(){
	...
	no_execute=
	memory=
	usage='usage: RunTranscriptXrefs [ -f "array_format" default=$ARRAY_FORMATS ]+ [ -d(elete xrefs) -s(kip backup) -h(elp) -l(ocal i.e. no bsub) -n(o execute i.e. just echo cmd) -m(emory in kb if array_nr fasta not available)]'
	
	OPTIND=1
	
	while getopts ":f:a:m:lnsdh" opt; do
		case $opt in 
	        f  ) formats="$OPTARG $formats";;
	...
	for format in $ARRAY_FORMATS; do
		...
		if [[ ! $names ]]; then
			...
			script_cmd="$EFG_PERL ${EFG_SRC}/scripts/array_mapping/probe2transcript.pl --species $SPECIES --transcript_dbname $DNADB_NAME --transcript_host $DNADB_HOST --transcript_port $DNADB_PORT --transcript_user $DNADB_USER $transpass --xref_host $DB_HOST --xref_dbname $DB_NAME --xref_user $DB_USER --xref_pass $DB_PASS $PROBE2TRANSCRIPT_PARAMS $delete_xrefs $MULTI_SPECIES -vendor $vendor -format $format -arrays $valid_names -import_edb"
