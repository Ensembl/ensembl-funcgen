...
#Remove ImportArrays QUEUE and RESOURCE env vars from config, as this is now written locally in batch_queue.pm
#Move a lot of this set up from ImportArrays to SetUpPipeline (or just re-engineer in hive script)
...
_InitEnv(){
	...
	_InitPipelineEnv || \
  	...
  	export INSTANCE_CONFIG=${BASH_SOURCE[1]}
  ...
  #[a-z] set definition is not safe here
	...
	if [ $? -ne 0 ]; then
		...
		if [ $? -ne 0 ]; then
			echo "Failed to find input data, please create and populate \$ARRAYS_HOME. See ensembl-funcgen/docs/array_mapping.txt for details"
		fi
	...
	do
		...
		then
			...
			CheckGlobalVariables DNADB_NAME DNADB_HOST BUILD
		...
		then
			...
			then
				...
				then
					...
					then
						echo "Would you like to use the following $seq_type file" "(y|n)"
...
BuildFastas(){
  ...
  \? ) echo $usage; exit 1;;
	...
	if [ $array_list ]; then
      ...
      CheckGlobalVariables array_formats
      cat $array_list
      ...
      for format in $ARRAY_FORMATS; do
		...
		#this is necessary to allow the grep below
          ...
          Execute ln -fs $format_fasta $WORK_DIR/arrays.${format}.fasta

          #Append only those which are not already present to avoid duplication

          for file in $(cat $format_home/arrays.list); do

              if [ ! -e $WORK_DIR/arrays.list ] || 
                  [ ! $(grep -E "^$file$" $WORK_DIR/arrays.list) ]; then

                  echo $file >> $WORK_DIR/arrays.list
              fi
          done

          #Note this list may still be inaccurate if some fastas have been built but them removed
...
UnzipCatFasta(){
    ...
    CheckGlobalVariables file format_fasta
...
SetAlignTypes(){
	...
	if [[ $align_types ]]; then
		...
		for type in $align_types; do
			ValidateGlobalVariable type VALID_ALIGN_TYPES
		done
...
SetArrayFormats(){
	...
	then
		...
		for format in $formats; do
		  ...
		  ValidateGlobalVariable format VALID_ARRAY_FORMATS
		done		 
...
SetUpPipeline(){
	echo ":: SetUpPipeline $*"
	#Separated these steps, such that we can reconfigure batch_queue.pm without
	#destroying existing pipeline DB.

	#Really need to capture -h and unknown opts here
...
ConfigurePipeline(){
	echo ":: ConfigurePipeline $*"

	#Can now remove ImportArrays env vars from config, as we write local batch_queue.pm
	#Just need to make sure we have done BuildFastas first, so we have access to the fasta files.
	#All this needs to move to a script!
	#Need to identify what need validating first, so we can do a pipeline wide validation 
	#before setting up pipeline?
	#i.e. do formats have valid limits
	...
	array_formats=
	skip_import=
	usage='usage: ConfigurePipeline [ -f format ]* [ -s(kip import set up) ] [ -h(elp) ]'
...
" >> $batch_queue_file
	...
	#Build multi-condition waits first
		...
		for format in $ARRAY_FORMATS; do
			...
			#Can remove these env vars from here if we
...
">> $analysis_conf_file
	...
	#"select[mem>6000] rusage[mem=6000]\" -M 6000000',
	...
	#analysis limits
	#ProbeTranscriptAlign affy < 500MB, affy_st <300MB, AGILENT < 1GB, ILLUMINA < 900MB, CODELINK?,  PHALANX < 1700MB
	#ProbeAlign, some requires 6GB? ILLUMINA < 1GB, Agilent < 1GB, Affy < 1/1.2/1.6/1.9/5GB, affy st <1.2GB, PHALANX <1.1GB, CODELINK < 900MB

	#Can we eval these into place?

	#Set to 2GB for now until we can wrap this up in a write_config script in the hive
   
	
	#When we move this to hive, failed jobs should retry with double the memory, if that is the cause of failure, so long as it doesn't exceed the spec of the queue

	#This will change between formats due to differences in alignment params and probe length
	...
	for format in $ARRAY_FORMATS; do
      local analysis_mem_varname=

		if [[ $align_types = *Genomic* ]]; then
        analysis_mem_varname="${format}_ProbeAlign_MEM"
        CheckGlobalVariables $analysis_mem_varname
        
        analysis_mem_value=$(eval "echo \$$analysis_mem_varname")
        analysis_mem_string="'${analysis_mem_value}MB','"$(($analysis_mem_value * 3))"MB','"$(($analysis_mem_value * 6))"MB'"
		#analysis_resource="${resource}&&(mem>$analysis_mem_value)]\" -R \"$rusage,mem=${analysis_mem_value}]\" -M $analysis_mem_value"
		analysis_resource="${resource}]\" -R \"$rusage]\""
		...
		
	{
	 ...
	 batch_size => 5,
	 retries    => 3,
     memory     => [$analysis_mem_string],
     resource    => '$analysis_resource',
     retry_resource =>'$analysis_resource',
	 cleanup => 'no',
	},"  >> $batch_queue_file
		...
		if [[ $align_types = *Transcript* ]]; then
			analysis_mem_varname="${format}_ProbeTranscriptAlign_MEM"
      CheckGlobalVariables $analysis_mem_varname
...
ImportArrays(){
	...
	CheckGlobalVariables ARRAY_FORMATS ARRAYS_HOME
	...
	#Can remove this loop if we harcode the QUEUE and RESOURCE env vars in SetUpPipeline local batch_queue
	#Would also need to change RAW_FASTA to be built dynamically, based on the logic name?
	#Implement this in the hive
	...
	for format in $ARRAY_FORMATS; do
        ...
        #Move this config stuff to SetUpPipeline
		...
		if [ $musage -gt $MAX_HUGE_MEM ]; then
			...
			queue=$HUGEMEM_QUEUE
			echo "\$MAX_NORMAL_MEM($MAX_NORMAL_MEM) for queue $NORMAL_QUEUE has been exceeded. Queue has been set to $queue"
      echo -e "WARNING:\tThis may require moving data if the $queue queue has restricted filesystem access"
      #This will go away when we import ST arrays directly?"
		...
		resource="-R \"select[mem>${musage}] rusage[mem=${musage}]\" -M $musage"
...
CreateAlignIDs(){
	...
	for format in $ARRAY_FORMATS; do
		...
		if [[ ! -f $NR_FASTA ]]; then
			...
			if [ ! $chunks_arg ]; then
				num_probes="$(grep '^>' $NR_FASTA | wc -l )"
				echo ": Found $num_probes $format NR probe sequences"
				
	        	#Validate vs DB
			    #This will not catch failed ImportArrays jobs, but will catch erroneously truncated
			    # or mismatched nr fasta files
				query="SELECT count(distinct p.probe_id) from array a, array_chip ac, probe p where a.array_id=ac.array_id and ac.array_chip_id=p.array_chip_id and a.class='${format}'"
				imported_probes=$(QueryVal OUT $query)
				
    			#QueryVal doesn't handle distinct very well
	            imported_probes=$(echo $imported_probes | sed -r 's/\s*p.probe_id\)\s+//')
				
				if [[ $num_probes -ne $imported_probes ]]; then
					echo -e "Found a mismatch between the number of $format imported probes:\t$imported_probes"
					echo -e "and those present in $NR_FASTA:\t$num_probes"
					exit
				fi

				#Could really do with matching dbIDs to, before we start the alignment jobs?
			fi
            ...
            #Calculate the number of chunks
				...
				if [ ! @anal_id ]; then
					echo "Failed to retrieve analysis_id from Submit_${logic_name}"
					exit 1
				fi
...
TestProbeAlign(){
	...
	CheckGlobalVariablesOrUsage "$usage" format align_type	
	ValidateGlobalVariableOrUsage "$usage" format VALID_ARRAY_FORMATS
	ValidateGlobalVariableOrUsage "$usage" align_type VALID_ALIGN_TYPES
	...
	#Set the logic name
	if [[ $align_type = "GENOMIC" ]]; then
		CheckGlobalVariables GENOMICSEQS
		logic_name="${format}_ProbeAlign";
	else
		CheckGlobalVariables TRANSCRIPTSEQS
		logic_name="${format}_ProbeTranscriptAlign";	
	fi
...
\? ) echo $usage; return 1;;
	...
	CheckGlobalVariablesOrUsage "$usage" format
...
RollbackArrays(){
	...
	usage='usage: RollbackArrays [-f format|ALL]+ [ -m mode e.g. probe2transcript | ProbeAlign | ProbeTranscriptAlign (default) | probe_feature | probe  -d(force delete) -k(eep probe2transcript xrefs) -v (vendor, only required if format exists on more than vendor platform, specify one format only) -h(elp) ]';
...
#Need to add logic_name support here?
...
Simply submits the ProbeAlign jobs defined by CreateAlignIDs';
	...
	ConvertTableEngine -t probe_feature -e InnoDB
	...
	CheckGlobalVariables align_types
	SetArrayFormats $formats
	...
	for format in $ARRAY_FORMATS; do
		...
		for align_type in $align_types; do
			...
			ValidateGlobalVariable align_type VALID_ALIGN_TYPES
			seqs_var="${align_type}SEQS" 
			CheckGlobalVariables $seqs_var
...
SetMusage(){
	...
	CheckGlobalVariables array_file factor
	...
	if [ $type ]; then
		...
		CheckGlobalVariables $seq_file_name
...
#todo run job to defined $PROBE2TRANSCRIPT_PARAMS
#once, before we run the jobs in parallel, and set, this in the env, 
#to pass to the probe2transcript jobs
...
RunTranscriptXrefs(){
	...
	memory=
	debug=
	usage='usage: RunTranscriptXrefs [ -f "array_format" default=$ARRAY_FORMATS ]+ [ -d(elete xrefs) -s(kip backup) -h(elp) -l(ocal i.e. no bsub) -n(o execute i.e. just echo cmd) -m(emory in kb if array_nr fasta not available) -b (debug mode) -i (skip probe_feature InnoDB to MyISAM convert)]'
	
	OPTIND=1
	
	while getopts ":f:a:m:lnbsdhi" opt; do
		case $opt in 
	        f  ) formats="$OPTARG $formats";;
	...
	#Likelihood is that all ProbeAlign steps have finished
	#so convert back to MyISAM by default

	if [ ! $no_convert ]; then
		ConvertTableEngine -t probe_feature -e MyISAM
	fi

	
	#Analyze & Optimize array tables
	tables="array array_chip probe probe_set probe_feature"

	for t in $tables; do
		OptimizeAnalyzeTable $t
	done
		...
		if [[ ! $names ]]; then
			...
			if [ -n "$BSUB_PROBE2TRANSCRIPT" ]; then
				...
				if [ $musage -gt $MAX_HUGE_MEM ]; then
					...
					#BSUB_CMD="-k \"${WORK_DIR}/checkpoints 1 method='blcr'\" -q $queue -R\"select[mem>$musage] rusage[mem=$musage]\" -M $musage -e ${work_dir}/${file_name}.err -o ${work_dir}/${file_name}.out cr_run " # -J $job_name"removed as this is now done in submitJob

				BSUB_CMD="-q $queue -R\"select[mem>$musage] rusage[mem=$musage]\" -M $musage -e ${work_dir}/${file_name}.err -o ${work_dir}/${file_name}.out"
			fi
...
GetProbesetXrefs(){
	...
	while getopts ":n:a:h" opt; do
		case $opt in 
	        ...
	        a  ) arrays="$OPTARG $arrays";;  
		    \? ) echo $usage; return 1;;
			h  ) echo $usage; return 0;;
		esac 
	done		
	
	CheckGlobalVariablesOrUsage "$usage" names			
