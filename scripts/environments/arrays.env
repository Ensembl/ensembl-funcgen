...
DumpSeq(){
	...
	if [ $seq_type = "GENOMIC" ]; then
 	...
 	#Can't do this if we have note made a full copy of the DB!!!
		bsub_cmd="-e ${dir}/genomic_seq.%J.err -o ${dir}/genomic_seq.%J.out -R\"select[mem>6000] rusage[mem=6000]\" -M6000 "
...
ConfigurePipeline(){
	...
	#These should be moved to arrays.config

	AFFY_UTR_ProbeAlign_MEM=2000
	#Most Affy < 1 GB (handful <1.2, <1.6, <1.9, <5GB(*2))
	AFFY_UTR_ProbeTranscriptAlign_MEM=700
	AFFY_ST_ProbeAlign_MEM=1500
	AFFY_ST_ProbeTranscriptAlign_MEM=500
	AGILENT_ProbeAlign_MEM=1200
	AGILENT_ProbeTranscriptAlign_MEM=1500
	ILLUMINA_WG_ProbeAlign_MEM=1000
	ILLUMINA_WG_ProbeTranscriptAlign_MEM=1000
	CODELINK_ProbeAlign_MEM=1000
	CODELINK_ProbeTranscriptAlign_MEM=1000
	PHALANX_ProbeAlign_MEM=1500
	PHALANX_ProbeTranscriptAlign_MEM=2000
	#LEIDEN, STEMPLE_LAB_SANGER, ILLUMINA_INFINIUM?
	#Other non-vertebrate?
	
	#The following were guess work and probably need revising
	LEIDEN_ProbeAlign_MEM=1000
	LEIDEN_ProbeTranscriptAlign_MEM=1000
	STEMPLE_LAB_SANGER_ProbeAlign_MEM=1200
	STEMPLE_LAB_SANGER_ProbeTranscriptAlign_MEM=1500

  WUSTL_ProbeAlign_MEM=1200
  WUSTL_ProbeTranscriptAlign_MEM=1500
...
">> $analysis_conf_file
	...
	for format in $ARRAY_FORMATS; do
		...
		if [[ $align_types = *Genomic* ]]; then
			...
			if [ ! $analysis_mem_value ]; then
				echo -e "Cannot find \$$analysis_mem_varname"
				analysis_resource="${resource}] ${rusage}]\""
			else
				#build resource string
				#need to add rusage in here
				#mem_kb=$(($analysis_mem_value * 1000))
