...
DumpSeq(){
	...
	if [ $seq_type = "GENOMIC" ]; then
 	...
 	#Can't do this if we have note made a full copy of the DB!!!
		bsub_cmd="-e ${dir}/genomic_seq.%J.err -o ${dir}/genomic_seq.%J.out -R\"select[mem>6000] rusage[mem=6000]\" -M6000000 "
		...
		bsub_cmd="-e ${dir}/transcript_seq.%J.err -o ${dir}/transcript_seq.%J.out -R\"select[mem>6000] rusage[mem=6000]\" -M6000000 "
...
ConfigurePipeline(){
	...
	AGILENT_ProbeTranscriptAlign_MEM=1200
	...
	for format in $ARRAY_FORMATS; do
		...
		if [[ $align_types = *Genomic* ]]; then
			...
			if [ ! $analysis_mem_value ]; then
				...
				analysis_resource="${resource}&&(mem>$analysis_mem_value)]\" -R \"$rusage,mem=${analysis_mem_value}]\" -M $mem_kb"
			fi
...
RollbackArrays(){
	...
	usage='usage: RollbackArrays [-f format]+ [ -m mode e.g. probe2transcript | ProbeAlign | ProbeTranscriptAlign (default) | probe_feature | probe  -d(force delete) -k(eep probe2transcript xrefs) -v (vendor, only required if format exists on more than vendor platform, specify one format only) -h(elp) ]';
