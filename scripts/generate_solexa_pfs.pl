...
my ($set_name, $data_version, $clobber);
my $user = "ensadmin";
my $host = 'localhost';
my $port = '3306';
...
my $rset = $rset_a->fetch_by_name_Analysis($set_name, $anal);

throw("No ResultSet with name $set_name") if ! $rset;
...
foreach my $chr (@chrs){
  ...
  foreach my $rf(@{$rset->get_ResultFeatures_by_Slice($slice)}){
  ...
  print "Loaded $cnt PredictedFeatures for $set_name on chromosome $chr\n";
