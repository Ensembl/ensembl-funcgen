...
my ($chips, $pass, $full_delete);
my ($exp_name, $dbhost, $dbname, $help, $man, $log_msg);
my $port = 3306;
my $user = 'ensadmin';

GetOptions (
			"experiment_name|e=s" => \$exp_name,
			"chip_ids|c=s"        => \@chips,
			"pass|p=s"            => \$pass,
			"port=s"              => \$port,
			"dbname|n=s"          => \$dbname,
...
throw('Must define a -dbhost parameter') if ! $dbhost;
...
my $exp_a = $d->get_ExperimentAdaptor();
...
throw("Experiment $exp_name does not exist in the database");
...
my @rsets = @{$rset_a->fetch_all_by_Experiment()};
...
foreach my $rset(@rsets){
  ...
  foreach my $ec(@{$rset->get_ExperimentalChips()}){
	...
	if(@chips){#delete only @chips
	  ...
	  if(! grep/$ec->unique_id()/, @chips){#other chips present, don't remove rset/cc
		$remove_cc = 0;
		$remove_rset = 0;
	  }
	}
  ...
  if(keys %cc_ids){#we have something to delete
	...
	$db->dbc->do($sql) || throw("Failed to delete chip_channel and results for cc_ids:\t".join(', ', values %cc_ids));
	

	#No do this after for all ecs/channels
	#roll back channel or experimental chip if not already deleted	
	#foreach my $dbID(keys %cc_ids){
	#  push @dbIDs, $dbID if(! exists $deleted{$table_name}{$dbID});
	#}
	
	#$sql = "DELETE from $table_name where ${table_name}_id IN (".join(', ', @dbIDs).")";
	#$db->dbc->do($sql) || throw('Failed to roll back ${table_name}s with dbID:\t('.join(', ', @dbIDs));
	...
	if($remove_rset){
	  ...
	  $db->dbc->do($sql) || throw("Failed to delete result_set and associated chip_channels with dbID:\t".$rset->dbID());

	  #delete status entries (should we do this first?)
	  $sql = 'DELETE from status where table_name="result_set" and table_id='.$rset->dbID()
	  $db->dbc->do($sql) || throw("Failed to delete status entries for result_set with dbID:\t".$rset->dbID());

	  
	  #remove data and feature sets if not linked to other experiments
	  if($full_delete){
		
		foreach my $dset(@{$dest_a->fetch_all_by_ResultSet()}){
