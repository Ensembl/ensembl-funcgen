...
sub resource_classes {
    ...
    return {
	    ...
	    4 => { -desc => 'long high memory',      'LSF' => '-q long -M5000000 -R"select[mem>5000] rusage[mem=5000]"' },    
	    5 => { -desc => 'long ens-genomics1 high memory',  'LSF' => '-q long -M5000000 -R"select[myens_genomics1<1000 && mem>5000] rusage[myens_genomics1=10:duration=10:decay=1:mem=5000]"' },
	    6 => { -desc => 'normal high memory',    'LSF' => ' -M5000000 -R"select[mem>5000] rusage[mem=5000]"' },    
	   };
}
  ...
  return [
	  {   
	   ...
	   -flow_into => {
			  1 => [ 'run_alignments' ],           
			  2 => [ 'wrap_up_pipeline' ],   
			  3 => [ 'converge_replicates' ],
			 },
	   # These jobs can be run in parallell... don't put too many since it may generate many jobs...	   
	   -limit => 1,
	   -batch_size => 1,
	   -hive_capacity => 10,
	   -rc_id => 0,
