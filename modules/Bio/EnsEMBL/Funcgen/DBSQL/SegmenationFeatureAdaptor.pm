...
#Can we remove some of the analysis checking from the AnnotatedFeatureAdaptor::store method?
...
sub store{
	...
	#experiment_id, segmentation_feature_id)
    #                          VALUES (?, ?)");
  ...
  FEATURE: foreach my $seg (@segs) {
		...
		if ( ! $seg->analysis->is_stored($db)) {
			throw('A stored Bio::EnsEMBL::Analysis must be attached to the SegmentationFeature objects to be stored.');
		}
		
		if (! $seg->feature_set->is_stored($db)) {
			throw('A stored Bio::EnsEMBL::Funcgen::FeatureSet must be attached to the SegmentationFeature objects to be stored.');
		}

		#sanity check analysis matches feature_set analysis
		if($seg->analysis->dbID() != $seg->feature_set->analysis->dbID()){
			throw("SegmentationFeature analysis(".$seg->analysis->logic_name().") does not match FeatureSet analysis(".$seg->feature_set->analysis->logic_name().")\n".
		...
		#This would still not be specific for each feature, nor would the segmentation_feature analysis_id
		#reflect all the combined analyses.  Maybe just the one which contributed most?

		# Store the analysis if it has not been stored yet
		#$analysis_adaptor->store( $seg->analysis()) if ( !$seg->analysis->is_stored($db) );
		...
		$sth->bind_param(5, $seg->feature_set->dbID(),   SQL_INTEGER);
		$sth->bind_param(6, $seg->display_label(),       SQL_VARCHAR);
		$sth->bind_param(7, $seg->score(),                SQL_DOUBLE);
		$sth->bind_param(8, $seg->summit,                SQL_INTEGER);

		$sth->execute();
		$seg->dbID( $sth->{'mysql_insertid'} );

		#foreach my $exp_id(@{$seg->experiment_ids()}){
