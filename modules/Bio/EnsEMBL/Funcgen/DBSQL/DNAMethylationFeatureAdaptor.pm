...

{
    ...
    unless ( ( -e $path ) && ( -r $path ) ) {
        throw(
"-BIG_BED argument ' $path ' is not a valid path or the file is not readable"
        );
    }
    ...
    if ( ( -B $path ) && ( $path =~ /\.bb$/ ) ) {
        ...
        if ($bed) {
            if ( !ref($bed) || !$bed->isa('Bio::DB::bbiFile') ) {
                throw(
"-BIG_BED argument ' $path ' could not be coerced into a Bio::DB::bbiFile object"
                );
            }
        }
...
=head2 get_default_DNAMethylationFeatures
  ...
  Example                : $dnamethfeatadaptor->get_default_DNAMethylationFeatures(-SLICE  => $slice);
...

{
    ...
    my $refdnamethylationfeatures =
      $self->get_DNAMethylationFeatures( -MIN_READ_DEPTH => 10, @_ );
...
=head2 get_DNAMethylationFeatures
  ...
  Example                : $dnamethfeatadaptor->get_DNAMethylationFeatures(
                           -SLICE          => $slice,
                           -MIN_READ_DEPTH => 10
                            );
...

{
    ...
    {
        throw(
"Bio::EnsEMBL::Funcgen::DBSQL::DNAMethylationFeatureAdaptor object required"
        );
    }
    my ( $slice, $min_read_depth ) =
      rearrange( [ 'SLICE', 'MIN_READ_DEPTH' ], @_ );
    ...
    my $list_head = $self->bigBed_File_Adaptor->bigBedIntervalQuery(
        $chr,
        $slice_start - 1,
        $slice_end + 1
    );
...
#my $strand= $temp[2] eq "+" ? 1 : ($temp[2] eq "-" ? -1 : (! defined $temp[2] ? 0 : $temp[2]));
            ...
            my $dnamethylationfeature =
              Bio::EnsEMBL::Funcgen::DNAMethylationFeature->new
...
"Bio::EnsEMBL::Funcgen::DBSQL::DNAMethylationFeatureAdaptor object required"
    ...
    my $methylated_reads    = sprintf "%d",
      ( ( $percent_methylation / 100 ) * $total_reads );
...
#set strand if defined in the bb file as +1 for '+' and -1 for '-' or 0 if not defined or the field itself if anything other than + or -
    my $strand =
      $bed_data->[5] eq "+" ? 1
      : ( $bed_data->[5] eq "-" ? -1
        : ( !defined $bed_data->[5] ? 0 : $bed_data->[5] ) );

    return ( $start, $end, $strand, $methylated_reads, $total_reads,
